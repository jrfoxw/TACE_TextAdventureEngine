...
input_=''
items = ['sword','food','pouch','cheese','shield','stick','gems']
...
## DEFINE LISTS #########################################
gold = []
commands = {'S':'search','G':'get','D':'drop','U':'use','I':'inventory','L':'look','P':'player'}
#########################################################
...
def get_command():
    ...
    if focus in area:
        ...
        area.remove(focus)
        carried = len(backpack)
        backpack.setdefault(carried+1,focus)
        print('DEBUG### carried value = {}'.format(carried))
    else:
...
def inventory_command():
    ...
    if len(backpack) >0:
        ...
        for item in backpack.keys():
            ...
            print('\t{:5} --- {}'.format(item,backpack[item]))
